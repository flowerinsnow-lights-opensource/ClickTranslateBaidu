...
import online.flowerinsnow.clicktranslate.client.object.TranslateLanguage;
import online.flowerinsnow.clicktranslate.client.object.TranslateResult;
import online.flowerinsnow.clicktranslate.client.object.json.TranslateResponse;
...
public abstract class TranslateUtils {
    ...
    public static TranslateResult translate(String message) {
        ...
        try {
            URL url = new URL(Config.API.getNotNull() + "?q=" + URLEncoder.encode(message, "UTF-8") + "&from=" + from.name + "&to=" + to.name + "&appid=" + appid + "&salt=" + new String(salt) + "&sign=" + md5(appid, message, new String(salt), Config.APP_SECRET.getNotNull()));
